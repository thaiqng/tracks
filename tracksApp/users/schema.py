...
class Query(graphene.ObjectType):
    ...
    def resolve_me(self, info):
        ...
        if user.is_anonymous: # if use is not authenticated
            raise Exception("Not signed in!") # to prevent this exception must pass through a valid jwt token on an auth header when sending a request to execute a graphql operation
        return user
